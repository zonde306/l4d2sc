...
void SpawnItems(const float vOrigin[3], const float vAngles[3], int index)
{
	...
	if( entity != -1 )
	{
		...
		CreateTimer(0.2, tmrSolidCollision, EntIndexToEntRef(entity));
	...
	if( iWitch == 1 )
	{
		...
		if( entity != -1 )
		{
			...
			SetEntProp(entity, Prop_Send, "m_CollisionGroup", 1);
			CreateTimer(0.3, tmrSolidCollision, EntIndexToEntRef(entity));
		}
	}
...
public Action tmrSolidCollision(Handle timer, any entity)
{
