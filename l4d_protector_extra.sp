...
public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	...
	if(dead_player>0)
	{
		...
		new GunData[GunInfo_t];
		int maxLength = g_hArrayGunList.Length;
		for(new i = 0; i < maxLength; ++i)
		{
			g_hArrayGunList.GetArray(i, GunData, GunInfo_t);
			if(GunData[GunEnemy] != dead_player)
				continue;
			
			GunData[GunEnemy] = 0;
			g_hArrayGunList.SetArray(i, GunData, GunInfo_t);
		}
	}
}
...
RemoveProtector(client, index = -1)
{
	...
	{
		g_hArrayGunList.GetArray(index, GunData, GunInfo_t);
...
FindClientGun(client, GunData[GunInfo_t], start = -1)
{
	int maxLength = g_hArrayGunList.Length;
...
FindGun(ent, GunData[GunInfo_t], start = -1)
{
	int maxLength = g_hArrayGunList.Length;
	for(int i = start + 1; i < maxLength; ++i)
	{
...
{
	new GunData[GunInfo_t];
	GunData[GunModelFoot] = -1;
	GunData[GunModelHead] = -1;
	GunData[GunModelOnBack] = -1;
	GunData[GunEnemy] = -1;
	...
	GoWork(client, g_hArrayGunList.PushArray(GunData, GunInfo_t));
}
GoBack(client, index = -1)
{
	...
	if(index == -1)
	{
		while((index = FindClientGun(client, GunData, index)) > -1)
		{
			...
			GunData[GunModelFoot] = -1;
			GunData[GunModelHead] = -1;
			GunData[GunModelOnBack] = CreateOnBack(client);
			
			g_hArrayGunList.SetArray(index, GunData, GunInfo_t);
		}
	}
	else
	{
...
GoWork(client, index = -1)
{
	...
	if(index == -1)
	{
		while((index = FindClientGun(client, GunData, index)) > -1)
		{
			...
			RemoveEnt(GunData[GunModelOnBack]);
			GunData[GunModelOnBack] = 0;
			
			GunData[GunModelFoot] = CreateFoot(gun_pos, client);
			GunData[GunModelHead] = CreateHead(GunData[GunModelFoot], client);
...
int GetBestTarget(int index, float gunPos[3], float gunAng[3])
{
	...
	data.WriteCell(GunData[GunOwner]);
	data.WriteCell(GunData[GunModelFoot]);
	data.WriteCell(GunData[GunModelHead]);
...
stock bool:IsValidEnemy(entity)
{
	if(g_bAttackSpecial)
	{
		if(IsValidAliveClient(entity) && GetClientTeam(entity) == 3)
		{
