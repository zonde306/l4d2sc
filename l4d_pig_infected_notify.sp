...
public Action:Timer_TankStumbleByDoorCheck(Handle:timer, any:client)
{
	...
	{
		CPrintToChatAll("{green}[迷惑行为] {olive}%N {default}玩门夹晕了 {green}Tank{default}.",client);
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if( IsWitch(GetEventInt(event, "attackerentid")) && victim != 0 && IsClientConnected(victim) && IsClientInGame(victim) && GetClientTeam(victim) == 3 )
	{
		if(!IsFakeClient(victim))//真人特感 player
			CPrintToChatAll("{green}[迷惑行为]{default} {red}萌妹 {default}使出 {olive}天马流星拳 {default}打死 队友.");
		else
			CPrintToChatAll("{green}[迷惑行为]{default} {red}萌妹 {default}使出 {olive}天马流星拳 {default}打死 AI队友.");
	...
	&& victim != 0 && IsClientConnected(victim) && IsClientInGame(victim) && GetClientTeam(victim) == 3)//特感自殺
	{
		...
		if(StrEqual(weapon,"entityflame")||StrEqual(weapon,"env_fire"))//地圖的自然火
			...
			return;
		else kill_weapon = "原地暴毙";//卡住了 由伺服器自動處死特感
			
		if(GetEntProp(victim, Prop_Send, "m_zombieClass") == 8)//Tank suicide
		{
			if(!IsFakeClient(victim))//真人SI player
				CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s {default}了.",kill_weapon);
			else
				CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s {default}了.",kill_weapon);
		}
			...
			if(!IsFakeClient(victim))//真人SI player
				CPrintToChatAll("{green}[迷惑行为] {red}%N{default} {olive}%s {default}了.",victim,kill_weapon);
			else
				CPrintToChatAll("{green}[迷惑行为] {red}%N{default} {olive}%s {default}了.",victim,kill_weapon);
	...
	{
		CPrintToChatAll("{green}[迷惑行为] {red}妹子{default} {olive}咬舌自尽 {default}了.");
	}
	...
	if( StrEqual(victimname,"Witch") && PlayerIsTank(attacker) )
	{
		...
		if(!IsFakeClient(attacker))//真人Tank player
			CPrintToChatAll("{green}[迷惑行为] Tank {default}使出 {olive}%s{default}•星爆气流斩 带走了 {red}妹子{default}.",Tank_weapon);
		else
			CPrintToChatAll("{green}[迷惑行为] Tank {default}使出 {olive}%s{default}•星爆气流斩 带走了 {red}妹子{default}.",Tank_weapon);
	...
	{	
		if(attacker != 0 && IsClientConnected(attacker) && IsClientInGame(attacker))//someone kill infected
		{
			...
			else if (PlayerIsTank(attacker))//Tank kill infected
			{
				...
				{
					if(!IsFakeClient(victim))//真人SI player
						CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s死 {default}队友.",Tank_weapon);
					else
						CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s死 {default}AI队友.",Tank_weapon);
				}
			}
		}
	}
...
public Action:Timer_SurKillBoomerCheck(Handle:timer, any:client)
{
	...
	if(SDKCall(g_hIsStaggering, Tankclient) && !surkillboomerboomtank && !tankstumblebydoor && !tankkillboomerboomhimself && !boomerboomtank)//tank在暈眩
	{
		...
		if(StrContains(model, "female") > 0)
			model = "肥婆";
		else
			model = "肥宅";
		
		if(!IsFakeClient(client))//真人boomer player
			CPrintToChatAll("{green}[迷惑行为] {olive}%N {default}打爆 {red}%N{default} %s 炸翻 {green}Tank{default}.",surclient,client,model);
		else
			CPrintToChatAll("{green}[迷惑行为] {olive}%N {default}打爆 {red}AI {default}%s 炸翻 {green}Tank{default}.",surclient,model);
...
public Action:Timer_TankKillBoomerCheck(Handle:timer, Handle:h_Pack)
{
	...
	if(SDKCall(g_hIsStaggering, Tankclient) && !surkillboomerboomtank && !tankstumblebydoor && !tankkillboomerboomhimself && !boomerboomtank)//tank在暈眩
	{
		if(!IsFakeClient(client))//真人SI player
			CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s死 {red}%N{default} %s 炸晕 {default}自己.",Tank_weapon,client,model);
		else	
			CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s死 {red}AI {default}%s 炸晕 {default}自己.",Tank_weapon,model);
	...
	{
		if(!IsFakeClient(client))//真人SI player
			CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s死 {default}%s.",Tank_weapon,model);
		else
			CPrintToChatAll("{green}[迷惑行为] {green}Tank {olive}%s死 {default}AI%s.",Tank_weapon,model);
	}
}
...
{	
	...
	{
		if(!IsFakeClient(client))//真人boomer player
			CPrintToChatAll("{green}[迷惑行为] {red}%N{default} %s 自爆了.",client,model);
		else
			CPrintToChatAll("{green}[迷惑行为] {red}AI {default}%s 自爆了.",model);
		return;
	}
	...
	{
		if(!IsFakeClient(client))//真人boomer player
			CPrintToChatAll("{green}[迷惑行为] {default}神队友 {red}%N{default} %s 炸翻 {green}Tank{default}.",client,model);
		else
			CPrintToChatAll("{green}[迷惑行为] {default}神{red}AI {default}%s 炸翻 {green}Tank{default}.",model);
		boomerboomtank = true;
		CreateTimer(3.0,COLD_DOWN,_);
	}
	else
	{
		if(!IsFakeClient(client))//真人boomer player
			CPrintToChatAll("{green}[迷惑行为] {red}%N{default}%s 原地爆炸了.",client,model);
		else
			CPrintToChatAll("{green}[迷惑行为] {red}AI {default}%s 原地爆炸了.",model);
	}
}
