...
public OnPluginStart()
{
	...
	g_cvarEnable = 				CreateConVar("l4d2_undoff_enable", 		"7", 	"启用的功能.1=贴脸黑枪.2=牛带人黑枪.4=机器人黑枪.7=全部", CVAR_FLAGS);
	g_cvarAnnounce = 			CreateConVar("l4d2_undoff_announce", 	"0", 	"黑枪提示.1=控制台.2=聊天框", CVAR_FLAGS);
	g_cvarBlockZeroDmg =		CreateConVar("l4d2_undoff_blockzerodmg","0", 	"阻止0伤害功能.1=取消统计数据.2=取消黑枪语音.4=取消机器人黑枪屏幕倾斜", CVAR_FLAGS);
	g_cvarDebug = 				CreateConVar("l4d2_undoff_debug", 		"0", 	"调试输出", CVAR_FLAGS);
	g_cvarPermDamageFraction = 	CreateConVar("l4d2_undoff_permdmgfrac", "1.0", 	"伤害作用于实血百分比", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	AutoExecConfig(true, "L4D2_UndoFF");
