...
// 按住这些按键可以冲刺
const int SPRINT_BUTTON = (IN_WALK|IN_ALT1|IN_ALT2|IN_SPEED);

// 这些按键被按住后无法冲刺
const int INVALID_SPRINT_BUTTON = (IN_BACK|IN_LEFT|IN_RIGHT|IN_MOVELEFT|IN_MOVERIGHT|IN_DUCK|IN_JUMP);

// 移动按钮
const int MOVING_BUTTON = (IN_FORWARD|IN_BACK|IN_LEFT|IN_RIGHT|IN_MOVELEFT|IN_MOVERIGHT|IN_JUMP);
...
float g_fCombatDelay, g_fThinkInterval, g_fSprintWalk, g_fSprintDuck, g_fSprintWater,
	...
	g_fSafeMagic, g_fBlockRevive, g_fCombatWillpower, g_fSafeWillpower, g_fCombatRadius;
...
ConVar g_pCvarAllow, g_pCvarDefenseFactor, g_pCvarDefenseChance, g_pCvarDefenseLimit,
	...
	g_pCvarThinkInterval, g_pCvarDamageMin, g_pCvarDefenseMin, g_pCvarBlockRevive, g_pCvarMenuFlush,
	g_pCvarWillpowerIdleRate, g_pCvarWillpowerRate, g_pCvarSprintAttack, g_pCvarSprintShove,
	g_pCvarSprintJump, g_pCvarSlotLevel, g_pCvarSlotCost, g_pCvarSlotMax, g_pCvarCombatRadius,
...
public void OnPluginStart()
{
	...
	g_pCvarCombatRadius = CreateConVar("sc2_combat_raduis", "500.0", "在多大范围内有敌人视为战斗状态", CVAR_FLAGS, true, 10.0);
	...
	HookEventEx("player_left_start_area", Event_RoundStart);
	HookEventEx("door_unlocked", Event_DoorUnlocked);
	HookEventEx("round_end", Event_RoundEnd);
	HookEventEx("map_transition", Event_RoundEnd);
	HookEventEx("mission_lost", Event_RoundEnd);
	HookEventEx("finale_win", Event_RoundEnd);
	...
	HookEventEx("bot_player_replace", Event_PlayerReplaceBot);
	HookEventEx("player_bot_replace", Event_BotReplacePlayer);
	...
	// 多语言支持
	LoadTranslations("simplecombat2.phrases.txt");
...
#define IsPlayerGhost(%1)				(HasEntProp(%1, Prop_Send, "m_isGhost") && GetEntProp(%1, Prop_Send, "m_isGhost", 1) != 0)
...
void ChooseSkill(int client, int page = 0, StringMap lastSkill = null)
{
	...
	char info[11];
	int length = g_hAllSkillList.Length;
	for(int i = 0; i < length; ++i)
	{
...
public Action Timer_OnMenuThink(Handle timer, any data)
{
	...
	for(int i = 1; i <= MaxClients; ++i)
	{
		...
		if(IsPlayerAlive(i) && !IsPlayerGhost(i) && g_fMagic[i] >= g_iStandingLimit && g_fNextStandingTime[i] <= time)
		{
			...
			if(HasEntProp(i, Prop_Send, "m_healthBuffer"))
			{
				float buffer = GetEntPropFloat(i, Prop_Send, "m_healthBuffer");
				canRevive = (health + buffer < maxHealth && (GetClientTeam(i) == 3 ||
...
public void UpdateMaxHealth(any client)
{
	...
	if(HasEntProp(client, Prop_Send, "m_isIncapacitated") &&
		(IsPlayerIncapacitated(client) || IsPlayerHanging(client)))
		return;
	...
	if(g_hCvarMaxHealth != null && g_hCvarMaxBufferHealth != null && g_hCvarMaxHealth.IntValue < newMaxHealth)
	{
...
stock bool CombatStart(int client)
{
	if(!IsValidAliveClient(client))
		return false;
...
stock bool CombatEnd(int client)
{
	if(!IsValidClient(client))
		return false;
...
// 增加玩家的耐力
stock float StaminaIncrease(int client, float amount)
{
	if(!IsValidClient(client))
		return -1.0;
...
// 减少玩家的耐力
stock float StaminaDecrease(int client, float amount)
{
	if(!IsValidClient(client))
		return -1.0;
...
// 增加玩家的魔力
stock float MagicIncrease(int client, float amount)
{
	if(!IsValidClient(client))
		return -1.0;
...
// 减少玩家的魔力
stock float MagicDecrease(int client, float amount)
{
	if(!IsValidClient(client))
		return -1.0;
...
// 增加玩家的精力
stock float WillpowerIncrease(int client, float amount)
{
	if(!IsValidClient(client))
		return -1.0;
...
// 减少玩家的精力
stock float WillpowerDecrease(int client, float amount)
{
	if(!IsValidClient(client))
		return -1.0;
...
stock bool GiveCash(int client, int& amount)
{
	if(!IsValidClient(client))
		return false;
...
stock bool GiveExperience(int client, int& amount)
{
	if(!IsValidClient(client))
		return false;
...
stock void GiveBonus(int client, int experience, int cash, const char[] reason = "", any ...)
{
...
stock bool IsVisibleThreats(int client)
{
	...
	if(HasEntProp(client, Prop_Send, "m_hasVisibleThreats"))
	{
	...
	if(target > 0 && IsValidEdict(target) && HasEntProp(target, Prop_Data, "m_iHealth") &&
		GetEntProp(target, Prop_Data, "m_iHealth") > 0 && GetTwoEntityDistance(client, target) <= g_fCombatRadius)
		return true;
	
	return FindEnemyInRange(client, g_fCombatRadius) > 0;
}

stock float GetTwoEntityDistance(int entity, int other)
{
	...
	GetEntPropVector(other, Prop_Send, "m_vecOrigin", dst);
	return GetVectorDistance(src, dst, false);
}

stock int GetCurrentAttacker(int client)
{
	if(!IsValidAliveClient(client) || !HasEntProp(client, Prop_Send, "m_tongueOwner"))
		return -1;
...
stock int FindEnemyInRange(int client, float radius = 300.0)
{
	...
	Handle trace = TR_TraceHullFilterEx(myOrigin, myOrigin, myMins, myMaxs,
		MASK_SHOT, TraceFilter_FindEnemyInRange, client);
...
#if defined _USE_DATABASE_MYSQL_
	SQL_FastQuery(g_hDatabase, "CREATE TABLE IF NOT EXISTS `user_info` (`uid` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, `sid` char(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT '', `sid2` char(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT '', `sid3` char(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT '', `name` text CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NULL DEFAULT NULL, `ip` char(16) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL, `country` char(64) CHARACTER SET utf8 COLLATE utf8_bin NULL DEFAULT NULL, `joindate` datetime(0) NOT NULL DEFAULT current_timestamp, `owner` char(20) CHARACTER SET ascii COLLATE ascii_bin NOT NULL DEFAULT '', `legit` int(10) UNSIGNED NOT NULL DEFAULT 0, PRIMARY KEY (`uid`) USING BTREE, UNIQUE INDEX `sid`(`sid`) USING BTREE, UNIQUE INDEX `sid2`(`sid2`) USING BTREE, UNIQUE INDEX `sid3`(`sid3`) USING BTREE) ENGINE = InnoDB AUTO_INCREMENT = 133 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;");
	SQL_FastQuery(g_hDatabase, "CREATE TABLE IF NOT EXISTS `user_online` (`uid` int(11) NOT NULL AUTO_INCREMENT, `coin` int(10) UNSIGNED NOT NULL DEFAULT 0, `country_name` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '', `region` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '', `city` varchar(64) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '', `code` char(3) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '', `code3` char(4) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '', `online` time(0) NOT NULL DEFAULT '00:00:00', `last` datetime(0) NOT NULL DEFAULT current_timestamp ON UPDATE CURRENT_TIMESTAMP, `alive` tinyint(1) UNSIGNED NOT NULL DEFAULT 0, PRIMARY KEY (`uid`) USING BTREE) ENGINE = InnoDB AUTO_INCREMENT = 133 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;");
	SQL_FastQuery(g_hDatabase, "CREATE TABLE IF NOT EXISTS `l4d2_simple_combat2` (`uid` int(10) UNSIGNED NOT NULL, `max_health` int(10) UNSIGNED NOT NULL DEFAULT 0, `max_stamina` int(10) UNSIGNED NOT NULL DEFAULT 0, `max_magic` int(10) UNSIGNED NOT NULL DEFAULT 0, `max_willpower` int(10) UNSIGNED NOT NULL DEFAULT 0, `accounts` int(11) NOT NULL DEFAULT 0, `points` int(11) NOT NULL DEFAULT 0, `level` smallint(5) UNSIGNED NOT NULL DEFAULT 0, `experience` int(10) UNSIGNED NOT NULL DEFAULT 0, `level_experience` int(10) UNSIGNED NOT NULL DEFAULT 0, `skill_slot` int(10) UNSIGNED NOT NULL DEFAULT 0, PRIMARY KEY (`uid`) USING BTREE) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;");
	SQL_FastQuery(g_hDatabase, "CREATE TABLE IF NOT EXISTS `l4d2_simple_combat_spell` (`uid` int(10) UNSIGNED NOT NULL, `spell` text CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, `skill` text CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL, PRIMARY KEY (`uid`) USING BTREE) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;");
#elseif defined _USE_DATABASE_SQLITE_
	SQL_FastQuery(g_hDatabase, "CREATE TABLE IF NOT EXISTS user_info (uid INTEGER PRIMARY KEY AUTOINCREMENT, sid CHAR(20) UNIQUE NOT NULL, sid2 CHAR(20) UNIQUE NOT NULL, sid3 CHAR(20) UNIQUE NOT NULL, name TEXT NOT NULL, ip CHAR(16) DEFAULT(''), country VARCHAR(32) DEFAULT(''), joindate DATETIME DEFAULT(datetime('now')), owner CHAR(20) DEFAULT(''), legit INT(10) DEFAULT(0));");
	SQL_FastQuery(g_hDatabase, "CREATE TABLE IF NOT EXISTS user_online (uid INTEGER PRIMARY KEY AUTOINCREMENT, coin INT(10) DEFAULT(0), country_name VARCHAR(64) DEFAULT(''), region VARCHAR(64) DEFAULT(''), city VARCHAR(64) DEFAULT(''), code CHAR(3) DEFAULT(''), code3 CHAR(4) DEFAULT(''), online TIME DEFAULT(time('0', '-12 hour')), last DATETIME DEFAULT(datetime('now')), alive INT(1) DEFAULT(0));");
...
public int SW_OnValidateClient(int ownerSteamId, int clientSteamId)
{
	...
	char steamId2Owner[64], steamId2Client[64];
	FormatEx(steamId2Owner, 64, "STEAM_1:%d:%d", (ownerSteamId & 1), (ownerSteamId >> 1));
	FormatEx(steamId2Client, 64, "STEAM_1:%d:%d", (clientSteamId & 1), (clientSteamId >> 1));
	SQL_FastQuery(g_hDatabase, tr("UPDATE user_info SET owner = '%s' WHERE sid2 = '%s';", steamId2Owner, steamId2Client));
	return 0;
}
...
#if defined _USE_DATABASE_MYSQL_ || defined _USE_DATABASE_SQLITE_
public void SQLTran_LoadPlayerComplete(Database db, any client, int numQueries,
	DBResultSet[] res, any[] unused)
{
	if(!IsValidClient(client))
		return;
...
public void SQLTran_LoadPlayerFailure(Database db, any client, int numQueries,
	const char[] error, int failIndex, any[] unused)
{
...
#if defined _USE_DATABASE_MYSQL_ || defined _USE_DATABASE_SQLITE_
public void SQLTran_SavePlayerComplete(Database db, any client, int numQueries,
	DBResultSet[] res, any[] unused)
{
	if(IsValidClient(client))
...
public void SQLTran_SavePlayerFailure(Database db, any client, int numQueries,
	const char[] error, int failIndex, any[] unused)
{
	if(failIndex == -1)
		return;
...
stock bool AddHealth(int client, int amount, bool limit = true)
{
	...
	if(HasEntProp(client, Prop_Send, "m_healthBuffer") && team == 2 &&
		!IsPlayerIncapacitated(client) && !IsPlayerHanging(client))
	{
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("SC_GiveClientBonus", Native_GiveClientBonus);
	CreateNative("SC_SetClientCombat", Native_SetClientCombat);
...
public int Native_GiveClientBonus(Handle plugin, int argc)
{
	if(argc < 4)
		ThrowNativeError(SP_ERROR_PARAM, "参数数量不足");
	...
	GiveBonus(client, experience, cash, reason);
}

public int Native_SetClientCombat(Handle plugin, int argc)
{
	if(argc < 2)
		ThrowNativeError(SP_ERROR_PARAM, "参数数量不足");
