...
***********************************************************************************************************************************************************************************/
Handle g_hCommandBotKicker = null;
...
stock CheatCommand( String:commandName[], String:argument1[] = "", String:argument2[] = "", bool:doUseCommandBot = false ) {
    ...
    if ( flags != INVALID_FCVAR_FLAGS ) {
		...
		if( doUseCommandBot ) {
			...
			if(g_hCommandBotKicker != null)
				g_hCommandBotKicker = KillTimer(g_hCommandBotKicker);
			g_hCommandBotKicker = CreateTimer(0.1, Timer_KickCommandBot, commandDummy);
		} else {
...
public Action Timer_KickCommandBot(Handle timer, any commandDummy)
{
	g_hCommandBotKicker = null;
	if(IsClientInGame(commandDummy))
		KickClient(commandDummy);
	return Plugin_Stop;
}
