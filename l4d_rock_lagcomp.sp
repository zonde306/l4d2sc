...
{
    cvarRockPrint = CreateConVar("sm_rock_print", "0", "显示伤害信息", FCVAR_NONE, true, 0.0, true, 1.0);
    cvarRockHitbox = CreateConVar("sm_rock_hitbox", "1", "使用自定义 hitbox", FCVAR_NONE, true, 0.0, true, 1.0);
    cvarRockTankLagComp = CreateConVar("sm_rock_lagcomp", "1", "是否开启延迟补偿", FCVAR_NONE, true, 0.0, true, 1.0);
    cvarRockGodframes = CreateConVar("sm_rock_godframes", "1.7", "投石多少秒内无敌", FCVAR_NONE, true, 0.0, true, 10.0);
    cvarRockGodframesRender = CreateConVar("sm_rock_godframes_render", "1", "投石无敌时间显示光圈", FCVAR_NONE, true, 0.0, true, 1.0);
    cvarRockHitboxRadius = CreateConVar("sm_rock_hitbox_radius", "30", "hitbox 范围", FCVAR_NONE, true, 0.0, true, 10000.0);
    
    cvarDamagePistol = CreateConVar("sm_rock_damage_pistol", "75", "手枪伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageMagnum = CreateConVar("sm_rock_damage_magnum", "1000", "马格南伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageShotgun = CreateConVar("sm_rock_damage_shotgun", "600", "霰弹枪伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageSmg = CreateConVar("sm_rock_damage_smg", "75", "冲锋枪伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageRifle = CreateConVar("sm_rock_damage_rifle", "200", "步枪伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageMelee = CreateConVar("sm_rock_damage_melee", "1000", "近战伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageSniper = CreateConVar("sm_rock_damage_sniper", "10000", "狙击伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    cvarDamageMinigun = CreateConVar("sm_rock_damage_minigun", "300", "固定机枪伤害", FCVAR_NONE, true, 0.0, true, DAMAGE_MAX_ALL_);
    
    cvarRangeMinAll = CreateConVar("sm_rock_range_min_all", "1", "攻击最小距离", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeMaxAll = CreateConVar("sm_rock_range_max_all", "2000", "攻击最大距离", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangePistol = CreateConVar("sm_rock_range_pistol", "2000", "手枪攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeMagnum = CreateConVar("sm_rock_range_magnum", "2000", "马格南攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeShotgun = CreateConVar("sm_rock_range_shotgun", "1000", "霰弹枪攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeSmg = CreateConVar("sm_rock_range_smg", "2000", "冲锋枪攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeRifle = CreateConVar("sm_rock_range_rifle", "2000", "步枪攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeMelee = CreateConVar("sm_rock_range_melee", "200", "近战攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeSniper = CreateConVar("sm_rock_range_sniper", "10000", "狙击枪攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
    cvarRangeMinigun = CreateConVar("sm_rock_range_minigun", "2000", "固定机枪攻击范围", FCVAR_NONE, true, 0.0, true, RANGE_MAX_ALL_);
	AutoExecConfig(true, "l4d_rock_lagcomp");
        ...
        rockEntity)
{
    ...
    if (ROCK_PRINT) {
        PrintToChatAll("武器: %s | 范围: %.2f", weaponName, range);
    }
...
public void ApplyBulletToRock(rockIndex, rockEntity, float damage, float range)
{
    ...
    if (ROCK_PRINT) {
        PrintToChatAll("石头血量: %d\%", RoundFloat(ROCK_HEALTH - rockDamage));
    }
}
...
public void PrintEntityLocation(int entity)
{
    if (IsValidEntity(entity)) {
        ...
        PrintToChatAll("实体 %s (%d) 的位置在: (%.2f, %.2f, %.2f)",
