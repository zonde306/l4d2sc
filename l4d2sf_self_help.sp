...
int g_iSlotHealing;
int g_iLevelSelfHelp[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	*/
	...
	LoadTranslations("l4d2sf_self_help.phrases.txt");
	
	HookEvent("player_spawn", Event_PlayerSpawn);
	HookEvent("player_first_spawn", Event_PlayerSpawn);
	
	g_iSlotHealing = L4D2SF_RegSlot("healing");
	L4D2SF_RegPerk(g_iSlotHealing, "self_help", 2, 50, 5, 2.0);
}

public Action L4D2SF_OnGetPerkName(int client, const char[] name, int level, char[] result, int maxlen)
{
	if(!strcmp(name, "self_help"))
		FormatEx(result, maxlen, "%T", "自救", client, level);
	else
...
{
	if(!strcmp(name, "self_help"))
		FormatEx(result, maxlen, "%T", tr("自救%d", IntBound(level, 1, 2)), client, level);
	else
		return Plugin_Continue;
	return Plugin_Changed;
}
...
{
	if(!strcmp(perk, "self_help"))
		g_iLevelSelfHelp[client] = level;
}

public void L4D2SF_OnLoad(int client)
{
	g_iLevelSelfHelp[client] = L4D2SF_GetClientPerk(client, "self_help");
}
...
{
	if(!g_bAllowedClient[client] && g_iLevelSelfHelp[client] <= 0)
		return false;
	
	bool bHasPA = CheckPlayerSupply(client, 4), bHasMedkit = CheckPlayerSupply(client, 3);
	
	if ((iUse == 1 || iUse == 3) && bHasPA && g_iLevelSelfHelp[client] >= 2)
	{
		return true;
	}
	else if ((iUse == 2 || iUse == 3) && bHasMedkit && g_iLevelSelfHelp[client] >= 1)
	{
		return true;
	}
