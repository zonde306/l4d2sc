...
int g_iSlotAbility;
int g_iLevelMovement[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	L4D2SF_RegPerk(g_iSlotAbility, "ability_movement", 2, 25, 5, 2.0);
}

public Action L4D2SF_OnGetPerkName(int client, const char[] name, int level, char[] result, int maxlen)
{
	if(!strcmp(name, "ability_movement"))
		FormatEx(result, maxlen, "%T", "特感能力移动", client, level);
	else
...
{
	if(!strcmp(name, "ability_movement"))
		FormatEx(result, maxlen, "%T", tr("特感能力移动%d", IntBound(level, 1, 2)), client, level);
	else
		return Plugin_Continue;
	return Plugin_Changed;
}
...
{
	if(!strcmp(perk, "ability_movement"))
		g_iLevelMovement[client] = level;
}
...
public void Event_PlayerSpawn(Event event, const char[] eventName, bool dontBroadcast)
{
	...
	g_iLevelMovement[client] = L4D2SF_GetClientPerk(client, "ability_movement");
}
