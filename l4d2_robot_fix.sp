...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("Robot_SetAllowedClient", NATIVE_Robot_SetAllowedClient);
	RegPluginLibrary("robot_helpers");
	return APLRes_Success;
}

public int NATIVE_Robot_SetAllowedClient(Handle plugin, int numParams)
{
	if(numParams < 2)
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_robots", sm_robot, ADMFLAG_CHEATS);
	RegConsoleCmd("sm_robot", sm_robot2, "创建护卫枪");
...
public Action SC_OnUseSpellPre(int client, char[] classname, int classnameMax)
{
	...
	if(robot[client]!=INVALID_ENT_REFERENCE)
	{
...
public Action L4D2_OnFindScavengeItem(int client, int &item)
{
	for(int i = 1; i <= MaxClients; ++i)
		if(robot[i] == EntIndexToEntRef(item))
			return Plugin_Handled;
...
public Event_RoundStart(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	{
		if(robot[i]!=INVALID_ENT_REFERENCE)
			Release(i, false);
...
Release(controller, bool:del=true)
{
	...
	if(r!=INVALID_ENT_REFERENCE)
	{
		robot[controller]=INVALID_ENT_REFERENCE;
...
public Action sm_robot2(int client, int argc)
{
	...
	if(robot[client]!=INVALID_ENT_REFERENCE)
	{
		PrintHintText(client, "你已经有一个 护卫枪 了！");
		return Plugin_Continue;
	}
	
	if(!g_bAllowCreator[client])
	{
		PrintToChat(client, "\x03[护卫枪]\x04 你\x01 没有「护卫」天赋或已经使用过了。");
		return Plugin_Continue;
	}
...
AddRobot(Client)
{
	...
	firetime[Client]=0.0;
	robot[Client]=EntIndexToEntRef(ent);
				...
				if(currenttime-firetime[Client]>chargetime)
				{
					...
					if( targetok)
					{
						if(bullet[Client]>0)
						{
							...
							FireBullet(Client, EntRefToEntIndex(robot[Client]), enemypos, robotpos);
...
ScanEnemy(Client, Float:rpos[3] )
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && GetClientTeam(i)==3 && IsPlayerAlive(i) && !IsPlayerGhost(i))
		{
			...
			GetEnemyPostion(i, infectedpos);
			// GetEntityAbsOrigin(i, infectedpos);
			...
			if(dis<=mindis)
			{
				...
				new Handle:trace = TR_TraceRayFilterEx(rpos, infectedpos, MASK_SHOT, RayType_EndPoint, TraceRayDontHitSelfAndSurvivor, EntRefToEntIndex(robot[Client]));
	...
	for(new iEntity = MaxClients + 1; iEntity <= iMaxEntities; iEntity++)
	{
			...
			(StrEqual(classname, "witch", false) && GetEntPropFloat(iEntity, Prop_Send, "m_rage") >= 1.0))
		{
			...
			// infectedpos[2]+=40.0;
			GetEnemyPostion(iEntity, infectedpos);
			// GetEntityAbsOrigin(iEntity, infectedpos);
...
FireBullet(controller, bot, Float:infectedpos[3], Float:botorigin[3])
{
	...
	for(new c=0; c<weaponbulletpershot[weapontype[controller]];c++)
	{
		...
		new hittarget=0;
		new Handle:trace = TR_TraceRayFilterEx(botorigin, vAngles2, MASK_SHOT, RayType_Infinite, TraceRayDontHitSelfAndSurvivor, bot);
		if(TR_DidHit(trace))
		{
			TR_GetEndPosition(pos, trace);
		...
		if((hittarget>0 && hittarget<=MaxClients) || IsCommonInfected(hittarget) || IsWitch(hittarget))
		{
			...
			*/
			...
			SDKHooks_TakeDamage(hittarget, 0, controller, weaponbulletdamage[weapontype[controller]] * RobotAttackEffect[controller], DMG_BULLET, bot, _, pos);
...
public bool:TraceRayDontHitSelfAndSurvivor(entity, mask, any:data)
{
	...
	/*
	else if(entity > MaxClients && HasEntProp(entity, Prop_Send, "m_hOwnerEntity"))
	{
