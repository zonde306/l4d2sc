...
int g_iSlotMelee;
int g_iLevelShove[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	LoadTranslations("l4d2sf_incap_shove.phrases.txt");
	
	HookEvent("player_spawn", Event_PlayerSpawn);
	HookEvent("player_first_spawn", Event_PlayerSpawn);
	
	g_iSlotMelee = L4D2SF_RegSlot("melee");
	L4D2SF_RegPerk(g_iSlotMelee, "incap_shove", 1, 25, 5, 2.0);
}

public Action L4D2SF_OnGetPerkName(int client, const char[] name, int level, char[] result, int maxlen)
{
	if(!strcmp(name, "incap_shove"))
		FormatEx(result, maxlen, "%T", "倒地推", client, level);
	else
...
{
	if(!strcmp(name, "incap_shove"))
		FormatEx(result, maxlen, "%T", tr("倒地推%d", IntBound(level, 1, 1)), client, level);
	else
		return Plugin_Continue;
	return Plugin_Changed;
}
