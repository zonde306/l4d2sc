...
public Hunter_OnModuleStart() {
	...
	hCvarFastPounceProximity = CreateConVar("ai_fast_pounce_proximity", "1000", "在距离敌人多远时强制进行飞扑");
	
	// Verticality
	hCvarPounceVerticalAngle = CreateConVar("ai_pounce_vertical_angle", "7", "弹跳垂直角度限制");
	
	// Pounce angle
	hCvarPounceAngleMean = CreateConVar( "ai_pounce_angle_mean", "10", "随机数生成器平均值" );
	hCvarPounceAngleStd = CreateConVar( "ai_pounce_angle_std", "20", "随机数生成器偏差值" );
	hCvarStraightPounceProximity = CreateConVar( "ai_straight_pounce_proximity", "200", "敌人多近的距离时尝试往上扑");
	
	// Aim offset sensitivity
	hCvarAimOffsetSensitivityHunter = CreateConVar("ai_aim_offset_sensitivity_hunter",
									"30",
									"被敌人看着的视野范围，在被看着时不要直接扑",
									FCVAR_NONE,
									true, 0.0, true, 179.0 );
	// How far in front of hunter to check for a wall
	hCvarWallDetectionDistance = CreateConVar("ai_wall_detection_distance", "-1", "检查墙壁距离.-1=禁用");
