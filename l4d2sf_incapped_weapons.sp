...
ArrayList g_aLv1Restrict, g_aLv0Restrict;
...
int g_iSlotSpecial;
int g_iLevelIncap[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	if( g_bLeft4Dead2 )
	{
		...
		g_hCvarPist =	CreateConVar(	"l4d_incapped_weapons_pistol",			"1",					"L4D2 only: 0=Don't give pistol (allows Melee weapons to be used). 1=Give pistol (game default).", CVAR_FLAGS);
	...
	g_aLv1Restrict = CreateArray();
	g_aLv1Restrict.Push(13);
	g_aLv1Restrict.Push(14);
	g_aLv1Restrict.Push(25);
	
	g_aLv0Restrict = CreateArray();
	g_aLv0Restrict.Push(13);
	g_aLv0Restrict.Push(14);
	g_aLv0Restrict.Push(25);
	g_aLv0Restrict.Push(2);
	g_aLv0Restrict.Push(3);
	g_aLv0Restrict.Push(4);
	g_aLv0Restrict.Push(5);
	g_aLv0Restrict.Push(6);
	g_aLv0Restrict.Push(7);
	g_aLv0Restrict.Push(8);
	g_aLv0Restrict.Push(9);
	g_aLv0Restrict.Push(10);
	g_aLv0Restrict.Push(11);
	g_aLv0Restrict.Push(21);
	g_aLv0Restrict.Push(26);
	g_aLv0Restrict.Push(33);
	g_aLv0Restrict.Push(34);
	g_aLv0Restrict.Push(35);
	g_aLv0Restrict.Push(36);
	g_aLv0Restrict.Push(37);
	...
	LoadTranslations("l4d2sf_incap_weapon.phrases.txt");
	
	g_iSlotSpecial = L4D2SF_RegSlot("special");
	L4D2SF_RegPerk(g_iSlotSpecial, "incap_weapon", 2, 75, 10, 2.0);
}
...
{
	if(!strcmp(name, "incap_weapon"))
		FormatEx(result, maxlen, "%T", "倒地武器", client, level);
	else
...
{
	if(!strcmp(name, "incap_weapon"))
		FormatEx(result, maxlen, "%T", tr("倒地武器%d", IntBound(level, 1, 2)), client, level);
	else
		return Plugin_Continue;
	return Plugin_Changed;
}
...
{
	if(!strcmp(perk, "incap_weapon"))
		g_iLevelIncap[client] = level;
}

public void L4D2SF_OnLoad(int client)
{
	g_iLevelIncap[client] = L4D2SF_GetClientPerk(client, "incap_weapon");
}
...
bool ValidateWeapon(int client, int weapon)
{
	...
	if(g_iLevelIncap[client] <= 0 && g_aLv0Restrict.FindValue(index) != -1)
		return false;
	
	if(g_iLevelIncap[client] == 1 && g_aLv1Restrict.FindValue(index) != -1)
		return false;
	
	if( index != 0 && g_aRestrict.FindValue(index) == -1 )
	{
		// SetEntPropEnt(client, Prop_Send, "m_hActiveWeapon", weapon);
		L4D2_ExecVScriptCode(tr("PlayerInstanceFromIndex(%d).SwitchToItem(\"%s\")", client, classname));
		return true;
	}
