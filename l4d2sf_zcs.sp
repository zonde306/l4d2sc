...
int g_iSlotAbility;
int g_iLevelSelect[MAXPLAYERS+1];
...
public void OnPluginStart()
{
	...
	g_hRespectLimits	= CreateConVar("zcs_respect_limits", "0", "Respect server configured z_versus limits.");
	...
	g_hAllowLastOnLimit	= CreateConVar("zcs_allow_last_on_limit", "1", "Allow player to select previous infected class when limits are up.");
	...
	LoadTranslations("l4d2sf_zombie_select.phrases.txt");
	
	// HookEvent("player_spawn", Event_PlayerSpawn);
	// HookEvent("player_first_spawn", Event_PlayerSpawn);
	
	g_iSlotAbility = L4D2SF_RegSlot("ability");
	L4D2SF_RegPerk(g_iSlotAbility, "zombie_select", 1, 50, 10, 2.0);
}

public Action L4D2SF_OnGetPerkName(int client, const char[] name, int level, char[] result, int maxlen)
{
	if(!strcmp(name, "zombie_select"))
		FormatEx(result, maxlen, "%T", "僵尸切换", client, level);
	else
...
{
	if(!strcmp(name, "zombie_select"))
		FormatEx(result, maxlen, "%T", tr("僵尸切换%d", IntBound(level, 1, 1)), client, level);
	else
		return Plugin_Continue;
	return Plugin_Changed;
}
...
{
	if(!strcmp(perk, "zombie_select"))
		g_iLevelSelect[client] = level;
}
...
{
	if (!g_bEnable || Client == 0 || !IsClientInGame(Client) || IsFakeClient(Client) || g_iLevelSelect[Client] < 1)
		return Plugin_Continue;
